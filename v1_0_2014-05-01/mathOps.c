...
void calcXtR(double *rTilde, double *vTilde, double *Fr, double *Hr, int ind, int numPart, double dt)
{
    ...
    if(ind == 0)
    {
        for(i = 0; i < numPart; i++)
        {
            ...
            rTilde[midIndX] = Fr[midIndX]*angs;		// set main unit system for rTilde to SI
            rTilde[midIndY] = Fr[midIndY]*angs;
            rTilde[midIndZ] = Fr[midIndZ]*angs;
        }
    }
    else
    {
        for(i = 0; i < numPart; i++)
        {
            ...
            rTilde[midIndX] = Fr[midIndX]*angs + rTilde[midPrevX] + vTilde[midPrevX]*dt + 0.5*dt*dt*Hr[midPrevX];
            rTilde[midIndY] = Fr[midIndY]*angs + rTilde[midPrevY] + vTilde[midPrevY]*dt + 0.5*dt*dt*Hr[midPrevY];
            rTilde[midIndZ] = Fr[midIndZ]*angs + rTilde[midPrevZ] + vTilde[midPrevZ]*dt + 0.5*dt*dt*Hr[midPrevZ];
        }
    }
}
...
void calcXNext(double *XNext, double *rTilde, double *vTilde, double *rNormal, double *vNormal, int numPart, int M)
{
    ...
    {
        XNext[2*(i/(3*numPart))*3*numPart+(i%(3*numPart))] = rNormal[i] - (rTilde[i]/angs);
