            ...
            while((tok = strsep(&temp," ")) != NULL)
            {
                if(j == 0)									// DON'T FORGET CHECKING MECHANISMS!!
                {
                    if(((i-1)/(M*numPart))+1 < histNum)
                        rHist[((i-1)/(M*numPart))+1][3*(i%(M*numPart))+j] = (double)(atof(tok));	// Added '+1' after (i-1)/(M*numPart) to update rHist & aHist at the same time!
                }
                else if(j == 1)
                {
                    if(((i-1)/(M*numPart))+1 < histNum)
                        rHist[((i-1)/(M*numPart))+1][3*(i%(M*numPart))+j] = (double)(atof(tok));
                }
                else if(j == 2)
                {
                    if(((i-1)/(M*numPart))+1 < histNum)
                ...
                {
                    if(((i-1)/(M*numPart))+1 < histNum)
                        aHist[((i-1)/(M*numPart))+1][3*(i%(M*numPart))+(j-3)] = (double)(atof(tok));
                }
                else if(j == 4)
                {
                    if(((i-1)/(M*numPart))+1 < histNum)
