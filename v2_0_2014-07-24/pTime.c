...
int main(int argc, char *argv[])
{
    ...
    int optInd,optCount,dtPathIndex;
    ...
    while(optInd < argc)
    {
        // [JUNE 5, 2014] - Added the --tr and --tv command line options for tolerances
        if(strcmp(argv[optInd],"--tr")==0)
        {
            tolR = (double)(atof(argv[optInd+1]));
            optInd += 2;
            optCount++;
        }
        else if(strcmp(argv[optInd],"--tv")==0)
        {
            tolV = (double)(atof(argv[optInd+1]));
            optInd += 2;
            optCount++;
        }
        ...
        else if(strcmp(argv[optInd],"--dt")==0)		// in atomic unit
        {
    ...
    // [C] May 21, 2014
    // Added this part for debugging purpose only. Remove if deemed debugged!
    /*printf("dt is %.2e\n",dt);
    printf("histNum is %d\n",histNum);
    printf("Path prefix is %s\n",argv[dirPrefixIndex]);
    printf("kValue is %d\n",kValue);
    printf("IValue is %d\n",IValue);
    printf("numPart is %d\n",numPart);*/
    ...
    for(i = 0; i < M; i++)
    {
        ...
        if(i == 0)
        {
            ...
            strcpy(pathStep0In,argv[dtPathIndex]);
            strcpy(pathForce0In,argv[dtPathIndex]);
            ...
            // [MAY 26, 2014] CONVERT x0, v0 into atomic units!
            // [MAY 28, 2014] Input files are already in atomic units! No need to convert!
        ...
        strcpy(pathTrajtIn,argv[dtPathIndex]);	// CHECK IF 'strcpy' replaces the new string to the old variable after the new round in the loop!!
        ...
        // MAY 21, 2014 LOG
        // Check if the input path is correct
        //printf("%s	%s\n",pathTrajtIn,pathForceIn);
        ...
        for(j = 0; j < numPart; j++)
        {
            ...
            sIndY = 3*j+1;
            sIndZ = 3*j+2;
            // CONVERT THE UNITS TO ATOMIC UNIT SYSTEM!!
            coord[midIndX] = tcoord[sIndX];//a0;
            coord[midIndY] = tcoord[sIndY];//a0;
            coord[midIndZ] = tcoord[sIndZ];//a0;
            vel[midIndX] = tvel[sIndX];//v_au;
            vel[midIndY] = tvel[sIndY];//v_au;
            vel[midIndZ] = tvel[sIndZ];//v_au;
            ...
            {
                massFac = mass[j]*toMe;
                Fr[midIndX] = coord[midIndX] - propagateX_au(x0[sIndX],v0[sIndX],f0[sIndX]/massFac,dt);			// AT THIS POINT, Fr are in angstroms!
                Fr[midIndY] = coord[midIndY] - propagateX_au(x0[sIndY],v0[sIndY],f0[sIndY]/massFac,dt);
                Fr[midIndZ] = coord[midIndZ] - propagateX_au(x0[sIndZ],v0[sIndZ],f0[sIndZ]/massFac,dt);
                Fv[midIndX] = vel[midIndX] - propagateV_au(v0[sIndX],f0[sIndX]/massFac,force[midIndX]/massFac,dt);
                Fv[midIndY] = vel[midIndY] - propagateV_au(v0[sIndY],f0[sIndY]/massFac,force[midIndY]/massFac,dt);
                Fv[midIndZ] = vel[midIndZ] - propagateV_au(v0[sIndZ],f0[sIndZ]/massFac,force[midIndZ]/massFac,dt);
            }
            else
            {
                massFac = mass[j]*toMe;
                ...
                Fr[midIndX] = coord[midIndX] - propagateX_au(coord[midPrevX],vel[midPrevX],force[midPrevX]/massFac,dt);	// AT THIS POINT, Fr are in angstroms!
                Fr[midIndY] = coord[midIndY] - propagateX_au(coord[midPrevY],vel[midPrevY],force[midPrevY]/massFac,dt);
                Fr[midIndZ] = coord[midIndZ] - propagateX_au(coord[midPrevZ],vel[midPrevZ],force[midPrevZ]/massFac,dt);
                Fv[midIndX] = vel[midIndX] - propagateV_au(vel[midPrevX],force[midPrevX]/massFac,force[midIndX]/massFac,dt);
                Fv[midIndY] = vel[midIndY] - propagateV_au(vel[midPrevY],force[midPrevY]/massFac,force[midIndY]/massFac,dt);
                Fv[midIndZ] = vel[midIndZ] - propagateV_au(vel[midPrevZ],force[midPrevZ]/massFac,force[midIndZ]/massFac,dt);
            }
        }
    }

    // May 21, 2014 Log
    // Check if Fv is not NaN
    ...
    // [MAY 26, 2014] - KEEP THIS CONVERGENCE CHECKING PROCEDURE FOR TESTING PURPOSE WITH ATOMIC UNITS FIRST!! IF STILL GIVES "NAN", CHECK rDiff[] INSTEAD!!
    //tolR = tol;
    //tolV = tol;
    ...
    strcpy(pathHistIn,argv[dtPathIndex]);
    strcat(pathHistIn,"history.hist");
    // [MAY 26, 2014] Passing atomic unit conversion factors instead of SI
    storeHist(rHist,aHist,coord,x0,force,f0,mass,toMe,a0,M,histNum,numPart,kValue,pathHistIn);
    freeChar1D(&pathHistIn);

    // MAY 21, 2014
    // Check validity of rHist and aHist
    /*for(i = 0; i < histNum; i++)
    {
        for(j = 0; j < 3*M*numPart; j++)
            printf("{%.2e,%.2e} ", rHist[i][j], aHist[i][j]);
        printf("\n");
    }*/
    ...
    setZero1D(rTilde,3*M*numPart);
    setZero1D(vTilde,3*M*numPart);
    ...
    strcpy(pathChunkOut,argv[dtPathIndex]);
    strcpy(pathTrajtOutConv,argv[dtPathIndex]);
    strcpy(pathNWOutConv,argv[dtPathIndex]);
    strcpy(pathForceOutConv,argv[dtPathIndex]);
    ...
    writeNW(tcoord,elemName,numPart,pathNWOutConv,pathForceOutConv,argv[dtPathIndex]);
    ...
    for(i = 0; i < M; i++)
    {
        strcpy(pathTrajtOutNConv,argv[dtPathIndex]);                     // Test in a simple program whether 'strcpy' replaces every time!!
        strcpy(pathNWOutNConv,argv[dtPathIndex]);
        strcpy(pathForceOutNConv,argv[dtPathIndex]);
        ...
        writeNW(tcoord,elemName,numPart,pathNWOutNConv,pathForceOutNConv,argv[dtPathIndex]);
    }

    strcpy(pathHist,argv[dtPathIndex]);
