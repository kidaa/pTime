...
int main(int argc, char *argv[])
{
    ...
    {
        massFac = mass[i]*toMe;
        coordNext[3*i] = propagateX_au(coord[3*i]/a0,vel[3*i]/v_au,force[3*i]/massFac,dt);
        coordNext[3*i] *= a0;
        coordNext[3*i+1] = propagateX_au(coord[3*i+1]/a0,vel[3*i+1]/v_au,force[3*i+1]/massFac,dt);
        coordNext[3*i+1] *= a0;
        coordNext[3*i+2] = propagateX_au(coord[3*i+2]/a0,vel[3*i+2]/v_au,force[3*i+2]/massFac,dt);
        coordNext[3*i+2] *= a0;
    }
