...
int main(int argc, char *argv[])
{
    ...
    int *ID,i,j,numPart,IValue,M,optInd,optCount,dtPrefixIndex;
    ...
    strcpy(pathTrajtIn,argv[dtPrefixIndex]);
    ...
    strcpy(pathNWOut,argv[dtPrefixIndex]);
    ...
    strcpy(pathForceOut,argv[dtPrefixIndex]);
    ...
    for(i = 0; i < M; i++)
    {
        ...
        strcpy(trajtFilenames[i],argv[dtPrefixIndex]);
        strcpy(nwFilenames[i],argv[dtPrefixIndex]);
        strcpy(forceFilenames[i],argv[dtPrefixIndex]);
        ...
        writeNW(coord,name,numPart,nwFilenames[i],forceFilenames[i],argv[dtPrefixIndex]);
    }

    // LAST UPDATED: May 1, 2014 --> Writing .nw file to the root directory as an initial config file for PTime
    writeNW(coord,name,numPart,pathNWOut,pathForceOut,argv[dtPrefixIndex]);
