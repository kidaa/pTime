...
int main(int argc, char *argv[])
{
    ...
    double dt,massFac,*coord,*vel,*velNext,*fPrev,*fNext,*mass,*massKg,kesum,keprevsum,keave,sum,Tave;
    ...
    {
        massFac = mass[i]*toMe;
        velNext[3*i] = propagateV_au(vel[3*i]/v_au,fPrev[3*i]/massFac,fNext[3*i]/massFac,dt);
        velNext[3*i+1] = propagateV_au(vel[3*i+1]/v_au,fPrev[3*i+1]/massFac,fNext[3*i+1]/massFac,dt);
        velNext[3*i+2] = propagateV_au(vel[3*i+2]/v_au,fPrev[3*i+2]/massFac,fNext[3*i+2]/massFac,dt);
