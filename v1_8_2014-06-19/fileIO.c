...
// [JUNE 9, 2014] - Using the most common isotope masses (addressing Cl-35 issue).
double elemMass[20] = {1.007825,4.002600,7.016000,9.012180,11.009310,12.000000,14.003070,15.994910,18.998400,19.992440,22.989800,23.985040,26.981540,27.976930,30.973760,31.972070,34.968850,39.962400,38.963710,39.962590};

// [JUNE 9, 2014] - Making readXYZ convert into a.u. right from the beginning.
...
void writeNW(double *coord, char **name, int numPart, char *pathNW, char *pathF, char *pathPrefix, char *dtPrefix)
{
    FILE *fp;
    int i;
    fp = fopen(pathNW,"w");
    ...
    fprintf(fp,"\n");
    fprintf(fp,"geometry units atomic noautoz noautosym noprint\n");
    for(i = 0; i < numPart; i++)
    ...
    fprintf(fp,"         print >> fileWrite, str(i+1) + ' ' + str((-1.0)*gradient[3*i]) + ' ' + str((-1.0)*gradient[3*i+1]) + ' ' + str((-1.0)*gradient[3*i+2])\n");
...
void writeTrajt(double *coord, int *ID, double *mass, double *vel, int numPart, const char *path)
{
    ...
    fp = fopen(path,"w");
    for(i = 0; i < numPart; i++)
        fprintf(fp,"%d %.6lf %.7e %.7e %.7e %.7e %.7e %.7e\n",ID[i],mass[i],coord[3*i],coord[3*i+1],coord[3*i+2],vel[3*i],vel[3*i+1],vel[3*i+2]);
    fclose(fp);
}
